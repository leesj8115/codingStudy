...
```
- 실행 결과  
![테이블 생성 확인](./Img/20210430/1.png)

## 2. no에 데이터 시퀀스를 참고하여 입력하기 위한 시퀀스를 만들어주세요.<br>(시퀸스명 : SETQ_TEST)
- 처리 방법  
좌측 메뉴 - 시퀸스 - 우클릭 하여 '새 시퀸스' 실행, 내용 입력  
 ![시퀸스 생성](./Img/20210430/21.png)
- 실행 결과  
 ![시퀸스 생성 결과](./Img/20210430/22.png)
...
```sql
insert into person
values (SETQ_TEST.nextval, 'test1', '1234', '010-1111-1111', sysdate);
```
- 실행 결과(`select * from person;`)  
![test1 삽입 결과](./Img/20210430/3.png)
...
values (SETQ_TEST.nextval, 'test2', '1234', '010-2222-2222', sysdate);
```
- 실행 결과(`select * from person;`)  
![test2 삽입 결과](./Img/20210430/4.png)
...
values (SETQ_TEST.nextval, '1234', 'test3');
-- 입력된 칼럼명 순서에 맞게 데이터 입력
```
- 실행 결과(`select * from person;`)  
![test3 삽입 결과](./Img/20210430/5.png)
...
```sql
UPDATE person
SET phone='010-3333-4444', reg = to_timestamp('20210501', 'YYYYMMDD')
where id='test3';
...
```
- 실행 결과(`select * from person;`)  
![test3 수정 결과](./Img/20210430/6.png)
...
```
- 실행 결과(`select * from person;`)  
![test2 삭제 결과](./Img/20210430/8.png)
...
```
- 실행 결과(`select * from person;`)  
![DB 미반영 후 복원](./Img/20210430/9.png)  
